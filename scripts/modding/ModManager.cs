...
internal partial class ModManager : Node
{
	...
	private readonly List<ModMetadata> LoadedMods = new();
	...
	private bool LoadMod(string dirName)
	{
		...
		ModLoadReport report = new(metadata.Name);
	...
	private bool LoadModAssembly(string path)
	{
		try
		{
			...
			AddChild(instance);
			MethodInfo load = instance.GetType().GetMethod("OnLoad");
			if (load != null)
			{
				load.Invoke(instance, null);
				GD.PushWarning("Mod " + path + " is using legacy methods (OnLoad). Inform the developer to update their mod!");
			}
	...
	private void LoadPresets(string root, ModLoadReport report)
	{
		...
		foreach (string preset in DirAccess.GetFilesAt(path))
		{
			...
			PresetManager.Instance.LoadModdedPreset(preset, report);
		}
	}
