...
internal partial class MenuManager : Node
{
	...
	private void RefreshEnergy(object sender, EventArgs e)
	{
		...
		EnergyBar.RegionRect = new Rect2(1, 1 + (float)Math.Ceiling(BattleManager.Instance.Energy / 3f) * 45f, 360f, 46f);
	}
	...
	private void MoveEnergyBarUp(bool immediate)
	{
		if (immediate)
		{
			EnergyBar.Position = new Vector2(320f, 363f);
		}
		else
		{
			EnergyBarTween?.Kill();
			EnergyBarTween = CreateTween();
			EnergyBarTween.TweenProperty(EnergyBar, "position", new Vector2(320f, 363f), 0.2f).SetTrans(Tween.TransitionType.Sine);
		}
	}
}
